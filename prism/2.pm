...
module p
        ...
        [p!q_m1] s_p=3 -> (s_p'=2);
        [p!q_m2] s_p=4 -> (s_p'=2);
        [pq] s_p=2 & (send' = 0) -> [0.2,0.3] : (s_p'=3) & (send = 1) & (m_p=1) + [0.7,0.8] : (s_p'=4) & (send = 1) & (m_p=2);
...
module q
        ...
        [rq] s_q=0 -> (s_q'=1);
        [sq] s_q=2 -> (s_q'=3);
        [p!q_m1] s_q=5 & (m_p = 1) -> (s_q'=4) & (send' = 0);
        [p!q_m2] s_q=5 & (m_p = 2) -> (s_q'=4) & (send' = 0);
        [s!q_m1] s_q=3 -> (s_q'=4) & (send' = 0);
        [r!q_m1] s_q=1 -> (s_q'=2) & (send'= 0);
...
p_min>=1: send => X receive

p_max=0: send ^ X not receive
